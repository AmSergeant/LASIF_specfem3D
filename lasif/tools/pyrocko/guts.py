...
for (style, cls) in list(str_style_map.items()):
    if style:
...
class DefaultMaker(object):
    ...
    def make(self):
        return self.cls(
            ...
            **dict((k, make_default(v)) for (k, v) in list(self.kwargs.items())))
...
class Constructor(object):
    ...
    def get_current_namespace_map(self):
        return dict((k, v[-1]) for (k, v) in list(self.namespaces.items()) if v)
...
def _dump_xml_header(stream, banner=None):
    ...
    stream.write(enc('<?xml version="1.0" encoding="UTF-8" ?>\n'))
