...
def stf_deconvolution(to_be_processed, output_folder, components = ['E','N','Z'],):  # NOQA
    ...
    for comp in components:    
        ...
        idx_sigwin_start = int(np.ceil((np.min(first_arrival) - process_params["seconds_prior_arrival"]) / process_params["dt"]))
        idx_sigwin_end = int(np.ceil((np.max(first_arrival) + process_params["window_length_in_sec"])/ process_params["dt"]))
        ...
        if not st_wav or not st_syn:
            ...
            stf, p, pp = source_deconvolution_freq(st_wav, st_syn,
                                                           lambd=0.001, recompute_syn=False)
