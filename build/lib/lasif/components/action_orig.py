...
class ActionsComponent(Component):
    ...
    def preprocess_data(self, iteration_name, components = ['E','N','Z'], svd_selection = False, noise_threshold=None, event_names=None):
            ...
            for event_name, event in iteration.events.items():
                ...
                for station_name, channels in  \
                    ...
                    for channel in location:
                        ...
                        tmp = channel["channel_id"].split('.')
                        channel_id_no_comp = tmp[0]+"."+tmp[1]+"."+tmp[2]
                        ret_dict = {
                            "process_params": process_params,
                            "input_filename": input_filename,
                            "output_filename": output_filename,
                            "channel_id": channel["channel_id"],
                            "channel_id_no_comp": channel_id_no_comp,
        ...
        print((to_be_processed[0:3]))

        '''
        #if 'R' in components or 'T' in components:
        stations = []
        for process in to_be_processed:
            processing_info = process["processing_info"]
            stations.append(processing_info["channel_id_no_comp"])
        stations = list(set(stations))
        for station in stations:
            input_filename_E = [process["processing_info"]["input_filename"]]
            
        '''
        #il faut donner a la preprocessing function les 3 composantes a traiter
    ...
    def stf_estimate(self, iteration_name, components = ['E','N','Z'], event_names=None):
        ...
        comp_to_process = components.copy()
        ...
        for event_name, event in iteration.events.items():
            ...
            stations_info = stf_deconvolution(one_event_to_be_processed, output_folder, components)
            #print(stations_info)
            #self.comm.visualizations.plot_stations(0)
